...
describe Polly::Calculation do
  ...
  it 'should perform a complex calculation' do
    @calc.final.should == 23500
  end

  it 'should perform a complex calculation' do
    @calc.h = "string"
    @calc.final.should == 19600
  end

  it 'should perform a complex calculation' do
    @calc.e = 20000
    @calc.final.should == 122300
  end
  ...
  it 'should convert the calculation to an array' do
    ...
    @calc.final.to_ary(numeric: true).should == \
      [:+, [:+, [:+, [100], [1000]], [:self, [17400]]], [5000]]
    @calc.final.to_ary(expand: true).should == \
      [:+, [:+, [:+, [100], [1000]], [:self, [:max, [:ceil, [:npv, [:round, [:a], [1]], [:c], [:br, [:>, [:length, [:h]], [4]], [:to_i, [:+, [:-, [:/, [:+, [:*, [:e], [:f]], [:g]], [3.0]], [:f]], [:-@, [:e]]]], [:e]]], [50]], [:e]]]], [:b]]
    @calc.final.to_ary(numeric: true, expand: true).should == \
      [:+, [:+, [:+, [100], [1000]], [:self, [:max, [:ceil, [:npv, [:round, [1.5], [1]], [12], [:br, [:>, [:length, ["str"]], [4]], [:to_i, [:+, [:-, [:/, [:+, [:*, [3000], [2]], [10000]], [3.0]], [2]], [:-@, [3000]]]], [3000]]], [50]], [3000]]]], [5000]]
  end
